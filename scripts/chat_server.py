...
DIAG_HINT = ("\n\n【見せ方は内容に応じて自分で判断せよ】答えを最も分かりやすく伝える形式を選ぶ:\n"
    ...
    "(サムネイルが表示される)。\n"
    "・🚫**画像ファイル名を推測・創作するな**。文脈/RAG検索結果に実ファイル名(例: `TKP_Tree.png`)が明示されている画像だけ "
    "`![](/asset/実ファイル名)` を出せ。名が確認できぬ画像は ![]() を書くな(存在せぬ名は404=割れ画像になる)。"
    "『あるはず』で `Nina_Unit_3D.png` 等の名を作るのは厳禁——実名が無ければ画像を出さず文で述べよ。\n"
